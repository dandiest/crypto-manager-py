# Crypto Portfolio Manager 🚀

A simple yet powerful Command Line Interface (CLI) tool built with **Python** and **SQLite** to manage and track cryptocurrency investments.

## ❗ [IMPORTANT] Disclaimer:
*This project is built strictly for educational purposes. It is a tool designed to demonstrate database management and Python logic. Do not use it as your primary source of financial tracking for real investments*.

## Features
- **Add Assets**: Save your coin purchases with quantity and buy price.
- **Auto-Calculation**: Automatically calculates the total value of each asset and the entire portfolio.
- **Persistence**: Data is saved locally in a SQLite database.
- **Clean UI**: Formatted tables for easy reading.

## How to use
1. Clone the repository: `git clone https://github.com/dandiest/CryptoPortfolioManager.git`
2. Run the script: `python crypto_manager.py`
3. Follow the on-screen menu.

## Tech Stack
- Python 3.x
- SQLite3
